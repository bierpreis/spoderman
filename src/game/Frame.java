...
public class Frame extends JFrame {
	...
	public Frame(Player player, Map map, int screenX, int screenY) {
		...
		setResizable(false);
		setVisible(true);
		System.out.println("Frame Ende konstruktor");
	...
	private class Screen extends JLabel {
		...
		protected void paintComponent(Graphics g) {
			...
			System.out.println("frame.paint component");
			g.setColor(Color.BLUE);
			for (int i = 0; i < map.getCube().length; i++) {

				g.fillRect((int) map.getCube()[i].getX(),
						(int) map.getCube()[i].getY(),
						(int) map.getCube()[i].getWidth(),
						(int) map.getCube()[i].getHeight());
			}
	...
	void sayMessage(boolean isSayMessage, Graphics g) { 
		if (isSayMessage) {
			...
			for (int i = 0; i < player.getMessagePics().length; i++) {
				g.drawImage(player.getMessagePics()[i], letterX[i], letterY[i],
						null);
	...
	void drawUnits(Graphics g) {
		...
		if (map.getSweg() != null)
			for (int i = 0; i < map.getSweg().length; i++) {
				g.drawImage(map.getSweg()[i].getLook(),
						map.getSweg()[i].getBounding().x,
						map.getSweg()[i].getBounding().y, null);
			}
		if (map.getBigmek() != null) {
			g.drawImage(map.getBigmek().getLook(),
					map.getBigmek().getBounding().x,
					map.getBigmek().getBounding().y, null);
		}
		if (map.getEnemies() != null)
			for (int i = 0; i < map.getEnemies().length; i++) {
				g.drawImage(map.getEnemies()[i].getLook(),
						map.getEnemies()[i].getBounding().x,
