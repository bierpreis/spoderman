...
public class Enemy {
	...
	public void update(boolean scrollingLeft, boolean scrollingRight) {
		...
		if (alive)
			topBounding = new Rectangle(bounding.x + 5, y, bounding.width - 8,
					10);
	...
	public BufferedImage getLook() {
			...
			if (alive)
				try {
					look = ImageIO.read(getClass().getClassLoader()
							.getResourceAsStream("img/Dolan.png"));
				} catch (IOException e) {
					e.printStackTrace();
				}
			else
				try {
					look = ImageIO.read(getClass().getClassLoader()
							.getResourceAsStream("img/blood.png"));
				} catch (IOException e) {
					e.printStackTrace();
				}
		}
		if (type == "gooby") {
			if (alive)
				try {
					look = ImageIO.read(getClass().getClassLoader()
							.getResourceAsStream("img/gooby.png"));
				} catch (IOException e) {
					e.printStackTrace();
				}
			else
